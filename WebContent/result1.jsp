...
<%
	int jid=Values.getJobId();
	System.out.println(jid);
	int cid=Values.getCid();
	int O=0,C=0,E=0,A=0,N=0;
	Connection con=DbConnection.connect();
	PreparedStatement psmtc=con.prepareStatement("select * from question");
	ResultSet qcount=psmtc.executeQuery();
	
	PreparedStatement psmt = con.prepareStatement("select * from question where qid=?");
	while(qcount.next()){
		
		psmt.setInt(1, qcount.getInt(1));
		ResultSet rs=psmt.executeQuery();
		rs.next();	
		String result=null;
		if(request.getParameter("q"+qcount.getInt(1)).equals("ha")){
			result=rs.getString(3);
		}
		else if(request.getParameter("q"+qcount.getInt(1)).equals("a")){
			result=rs.getString(4);
		}
		else if(request.getParameter("q"+qcount.getInt(1)).equals("d")){
			result=rs.getString(5);
		}
		else if(request.getParameter("q"+qcount.getInt(1)).equals("hd")){
